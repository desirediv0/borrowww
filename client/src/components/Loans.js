...
export default function Loans() {
  ...
  useEffect(() => {
    const updateScroll = () => {
      ...
      const scrollProgress = Math.max(0, Math.min(1, (windowHeight - rect.top) / (windowHeight * 1.5)))
      ...
      if (scrollProgress >= 0.3) {
        ...
        setCardStates(prevStates => prevStates.map((card, index) => {
            ...
            >
              <div className="bg-white rounded-xl p-6 max-w-xs border border-white/20 hover:bg-white shadow-lg hover:shadow-xl transition-all duration-300 group">
                <div className="flex items-center justify-between mb-4">
                  <div className={`text-2xl p-2 rounded-lg bg-gradient-to-r ${card.color} text-white w-12 h-12 flex items-center justify-center`}>
                    {card.icon}
                  </div>
